...
### Filtering
```{r feature_filtering}
hsc_s <- subset(x = hsc_s, subset = 100000 > nCount_RNA & percent_mt < 8)
...
# Plotting gois
```{r top_upregulated_cluster_8, fig.width=20, fig.height=15}

p1 <- FeaturePlot(hsc_s, features = markers_all[order(avg_log2FC, decreasing = T), .SD[1:19] ,by = cluster][p_val < 5e-2][order(cluster)][cluster == 8, gene], slot = "scale.data", order = T)
p2 <- DimPlot(hsc_s, label = T) + ggtitle("Clusters") + theme(plot.title = element_text(hjust = 0.5, size = 18))
...
```{r cell_cycle, fig.width=15, fig.height=10}


hsc_s <- CellCycleScoring(hsc_s, g2m.features = cc.genes$g2m.genes,
    s.features = cc.genes$s.genes)

p1 <- FeaturePlot(hsc_s, features = c("S.Score", "G2M.Score"))
p2 <- DimPlot(hsc_s,  group.by = "Phase")
...
```{r markers_luca_et_al, fig.width=15, fig.height=15}

goi_luca <- c("PLEK", "HBB", "MPO", "SPIB", "CD79A", "DNTT", "CD34", "CD164")
# goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")

p1 <- FeaturePlot(hsc_s, features = goi_luca, order = T, keep.scale = "all")
p2 <- DimPlot(hsc_s, label = T)
...
```{r long_lasting_hsc_markers, fig.width=10, fig.height=10, eval = F}

goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")


p1 <- FeaturePlot(hsc_s, features = goi_long_mark, slot = "data")
p2 <- DimPlot(hsc_s, label = T)
...
```{r long_lasting_hsc_markers_dif_names_counts_ordered, fig.width=15, fig.height=15}

goi_long_mark <- c("CD38", "PTPRC", "ITGA6", "THY1")
# goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")

p1 <- FeaturePlot(hsc_s, features = c(goi_long_mark, "CD34", "MPO"), slot = "scale.data", order = T, pt.size = 1, raster.dpi = c(100000, 100000))
p2 <- DimPlot(hsc_s, label = T)
...
```{r long_lasting_hsc_markers_dif_names_counts, fig.width=15, fig.height=15}

goi_long_mark <- c("CD38", "PTPRC", "ITGA6", "THY1")
# goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")

p1 <- FeaturePlot(hsc_s, features = c(goi_long_mark, "MPO", "CD34"), slot = "data", order = F, keep.scale = "all")
p2 <- DimPlot(hsc_s, label = T)
...
p1 <- FeaturePlot(hsc_s, features = c(goi_long_mark, "CD34"), slot = "data", order = T, keep.scale = "all")
p2 <- DimPlot(hsc_s, label = T)
...
```{r long_lasting_hsc_markers_dif_names_counts, fig.width=10, fig.height=15}

goi_long_mark <- c("CD38", "PTPRC", "ITGA6", "THY1")
# goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")

p1 <- FeaturePlot(hsc_s, features = c(goi_long_mark, "CD34"), slot = "data", order = F, keep.scale = "all")
p2 <- DimPlot(hsc_s, label = T)
...
```{r long_lasting_hsc_markers_dif_names_counts_ordered_mpo, fig.width=10, fig.height=15}
...
FeaturePlot(subset(hsc_s, subset = seurat_clusters == 8), features = c(goi_long_mark, "CD34") , split.by = "orig.ident", keep.scale = "all")
...
p1 <- FeaturePlot(subset(hsc_s, subset = orig.ident != "a"), features = c(goi_long_mark, "MPO", "CD34"), slot = "data", order = T, keep.scale = "all")
p2 <- DimPlot(hsc_s, label = T)
...
VlnPlot(subset(hsc_s, subset = seurat_clusters == 8), features = c(goi_long_mark) , split.by = "orig.ident")
...
```{r Mikkola et al., fig.width=15, fig.height=15}

goi_mikkola <- c("CD34", "RUNX1", "HOXA9", "MLLT3", "MECOM", "HLF")
# goi_long_mark <- c("CD38", "CD45RA", "CD49f", "CD90")

p1 <- FeaturePlot(hsc_s, features = goi_mikkola, slot = "scale.data", order = T)
p2 <- DimPlot(hsc_s, label = T) + ggtitle("Clusters") + theme(plot.title = element_text(hjust = 0.5, size = 18)
)

goi_mikkola_plot <- p1 + p2
goi_mikkola_plot
...
goi_10_25 <- c("SDC1", "SDC2", "SDC3", "SDC4", "SLC22A4", "FLT3", "TPO", "SCF", 
               ...
               "FZD3", "FZD4", "FZD5", "FZD6", "FZD7", "Frizzled 8", "FZD9", "FZD10",
               "CDH1", "CDH2", "CDH3", "CDH4", "CDH5")

p1 <- FeaturePlot(hsc_s, features = goi_10_25, slot = "data", order = T)
p2 <- DimPlot(hsc_s, label = T) + ggtitle("Clusters") + theme(plot.title = element_text(hjust = 0.5, size = 18)
)
...
# Integrate data from Myers et al.

```{r load myers et al., cache = F}
myers <- ReadMtx("data/myers_et_al/E-MTAB-9067.aggregated_filtered_counts.mtx",
        "data/myers_et_al/E-MTAB-9067.aggregated_filtered_counts.mtx_cols",
        "data/myers_et_al/E-MTAB-9067.aggregated_filtered_counts.mtx_rows")
```
...
```{r }
gt <- data.table("Geneid" = rownames(myers)) # gene title
...
```{r myers_seurat}
myers_s <- CreateSeuratObject(myers, project = "myers")
rm(myers)
```
...
```{r myers_s filtering}
print("nCount_RNA")
length(WhichCells(myers_s, expression = nCount_RNA > 10000))
length(WhichCells(myers_s, expression = nCount_RNA < 10000))


print("nFeature_RNA")
length(WhichCells(myers_s, expression = nFeature_RNA > 1000))
length(WhichCells(myers_s, expression = nFeature_RNA < 1000))

```


```{r percentage_features_myers}
myers_s <- PercentageFeatureSet(myers_s, pattern = "^MT-", col.name = "percent_mt")
myers_s <- PercentageFeatureSet(myers_s, "^RP[SL]", col.name = "percent_ribo")
myers_s <- PercentageFeatureSet(myers_s, "^HB[^(P)]", col.name = "percent_hb")
myers_s <- PercentageFeatureSet(myers_s, "PECAM1|PF4", col.name = "percent_plat")
```


```{r qc_myers, fig.width=5, fig.height=2.5}
VlnPlot(myers_s, features = c('nCount_RNA','nFeature_RNA'), split.by = "orig.ident", pt.size = .1, raster = F, log = T) +  NoLegend()

```


```{r myers_qc_main, fig.width=5, fig.height=5}
VlnPlot(myers_s, features = c('nCount_RNA','nFeature_RNA', 'percent_mt', 'percent_hb', "percent_ribo", "percent_plat"), split.by = "orig.ident", pt.size = .1, raster = F) +  NoLegend()

```

```{r feature_filtering_myers}
myers_s <- subset(x = myers_s, subset = nCount_RNA > 10000 & nFeature_RNA > 1000 & percent_mt < 18)
```
...
``` {r umi-features, fig.width = 10, fig.height = 5}
FeatureScatter(myers_s, "nCount_RNA", "nFeature_RNA", pt.size = 1, plot.cor = T) + scale_x_continuous(labels = scales::scientific) + NoLegend()

```


``` {r most_expressed, fig.width = 10, fig.height = 7}
counts_cells <- myers_s@assays$RNA@counts
counts_cells <- Matrix::t(Matrix::t(counts_cells)/Matrix::colSums(counts_cells)) * 100
...
```{r pca_myers}
myers_s <- SCTransform(myers_s, vst.flavor = "v2", verbose = FALSE) %>%
  RunPCA(npcs = 30, verbose = FALSE) %>%
  RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE)
...
```{r umap_1_myers,  fig.width= 15, fig.height= 10}
DimPlot(myers_s, pt.size = 2)
```


### remove ribosomal genes - obsolete

```{r remove_ribo_myers, eval = F}
counts_myers <- GetAssayData(myers_s, assay = "RNA") # get the matrix from the assay data
ribo_myers <- which(rownames(counts_myers) %like% "^RP[SL]") # find row numbers of ribosomal genes

counts_myers <- counts_myers[-ribo_myers,]

myers_s <- subset(myers_s, features = rownames(counts_myers))
```

### Removal of genes not present in the 10X panel used with the initial dataset

```{r remove_not_10X}
panel_10x <- fread("data/gene_list_10X_panel.txt", header = F)
counts_myers <- GetAssayData(myers_s, assay = "RNA") # get the matrix from the assay data
# get the indices of genes present in the 10X panel
in_panel_10x <- which(rownames(myers_s@assays$RNA) %in% panel_10x[, V1])

counts_myers <- counts_myers[in_panel_10x,]

# counts_myers <- counts_myers[-hsc_counts_0,]
myers_s <- subset(myers_s, features = rownames(counts_myers))
```
...
#Myers et al.
print("Myers dataset")
length(which(rownames(myers_s@assays$RNA) %in% panel_10x[, V1]))
```
...
### Label cells with cluster names from Myers et al.

```{r get the clustering data}

clust_myers <- fread("data/myers_et_al/E-MTAB-9067.clusters.tsv", sep = "\t")
...
```{r split and integrate patient single-cell with Myers et al. data, cache = T, results = 'hide', message = F, error = F, warning = F}
integration_list <- list(myers_s, hsc_s)
...
hsc_myers <- IntegrateData(anchorset = integration_anchors, normalization.method = "SCT")
```

## Dim Reduc
```{r, cache = T, results = 'hide', message = F, error = F, warning = F}

DefaultAssay(hsc_myers) <- "integrated"

hsc_myers <- SCTransform(hsc_myers, vst.flavor = "v2", verbose = FALSE) %>%
  RunPCA( npcs = 30, verbose = FALSE) %>%
  RunUMAP(reduction = "pca", dims = 1:30, verbose = FALSE)
...
DimPlot(hsc_myers, group.by = "orig.ident")
```

### PCA
```{r, cache = T, fig.width= 15, fig.height= 10}
DimPlot(hsc_myers, group.by = c("pat", "diag", "type", "rep"), reduction = "pca") + labs(tittle = "PCA")
```
