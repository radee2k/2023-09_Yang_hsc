...
hsc_s <- merge(d1, y = d2, project = "2023-09_Yang_hsc")
hsc_s <- merge(hsc_s, y = d3, project = "2023-09_Yang_hsc")
hsc_s <- merge(hsc_s, y = d4, project = "2023-09_Yang_hsc")
...
DimPlot(hsc_s, split.by = "orig.ident", pt.size = 2, group.by = "orig.ident")
```
...
DoHeatmap(hsc_s, group.by = "seurat_clusters", features = highest_de$gene, angle = 0, hjust = .5) + 
  NoLegend() + 
  scale_fill_gradient2("Average log2 fold change", low = heat_cols[1], mid = heat_cols[2], 
...
```{r save the data}
SaveH5Seurat(hsc_s, filename = "data/hsc_final.h5Seurat", overwrite = TRUE)
```
...
p3 <- DimPlot(hsc_s, label = T, label.size = 5)
