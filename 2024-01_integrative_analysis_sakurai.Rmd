...
wd <- "/disk2/user/radgro/projects/2023-09_Yang_hsc/2024-01_integrative_analysis_sakurai"
...
knitr::opts_chunk$set(dev = 'svglite', dev.args = list(system_fonts = fonts),
                      cache.path = "2024-01_integrative_analysis_sakurai_files/gfm", cache = F,
...
sakurai_s <- readRDS("data/GSE192519_Integrated_hHSC.rds")
```


```{r load sakurai et al. - initial data after merge, cache = F, include=F, eval=F}
sakurai <- ReadMtx("data/soluplus_mtx/GSM5750534_Soluplus_matrix.mtx.gz",
                   "data/soluplus_mtx/GSM5750534_Soluplus_barcodes.tsv.gz",
                   "data/soluplus_mtx/GSM5750534_Soluplus_features.tsv.gz",
                   feature.column = 1)
```
...
``` {r most_expressed, fig.width = 7.5, fig.height = 5}
...
```{r pca_sakurai_after_integration, warning = F, message = F}
...
```{r umap_1_sakurai_after_integration}
DimPlot(sakurai_s, label = T)
```
...
```{r split and integrate patient single-cell with sakurai et al. data, cache = T, results = 'hide', message = F, error = F, warning = F}
...
cluster_hsc_sak <- as.data.table(hsc_sakurai@meta.data$seurat_clusters)
